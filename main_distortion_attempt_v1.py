...
#    visionTable = nt.getTable('SmartDashboard')
   ...
   img = np.zeros(shape=(480, 680, 3), dtype=np.uint8)
   ...
   while True:
      ...
      frame_time, input_img1 = input_stream.grabFrame(img)
      ...
      input_img = cv2.remap(input_img1, map1, map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
