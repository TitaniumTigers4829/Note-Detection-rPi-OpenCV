...
def main():
   ...
   camMatrix = np.array([
    [258.53369267, 0., 322.63406518],
    [0., 259.34097252, 239.69189262],
    [0., 0., 1.]])
   
   distCoeff = np.array([
    [-3.21955766e-01, 1.03908909e-01, 2.12815879e-04, -5.75005502e-04, -1.47855098e-02]])
...
#    visionTable = nt.getTable('SmartDashboard')
   ...
   while True:
      ...
      # Undistort the image
      #input_img = cv2.undistort(input_img,camMatrix,distCoeff,None,camMatrixNew)
      ...
      if contour is not None and contour_is_note(contour):
         cv2.ellipse(input_img, cv2.fitEllipse(contour), (255, 0, 255), 2)
         ...
         #image_points = np.array([[[x_center, y_center]]], dtype=np.float32)  # Image points
         #undistorted_points = cv2.fisheye.undistortPoints(image_points, camMatrixNew, distCoeff)
